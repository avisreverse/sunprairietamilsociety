...
export default function AdminAchievementsPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (achievementsRes.ok) setItems(await achievementsRes.json());
    if (settingsRes.ok) {
      const settings: { key: string; value: string }[] = await settingsRes.json();
      const flag = settings.find((s) => s.key === "achievements_section_enabled");
  ...
  /**
   * Toggles the achievements_section_enabled flag in site_settings.
  ...
  const toggleSection = async () => {
    ...
    await fetchAdmin("/api/admin/site-settings", {
      ...
      body: JSON.stringify({ key: "achievements_section_enabled", value: next ? "true" : "false" }),
    });
    setSectionEnabled(next);
    console.log(`✅ [AdminAchievements] achievements_section_enabled → ${next}`);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#111010" }}>
      ...
      <main style={{ maxWidth: "1100px", margin: "0 auto", padding: "2.5rem 2rem" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "1rem", background: "rgba(255,255,255,0.04)", border: `1px solid ${sectionEnabled ? "rgba(29,209,161,0.25)" : "rgba(231,76,60,0.25)"}`, borderRadius: "12px", padding: "1rem 1.25rem", marginBottom: "1.5rem" }}>
          <label style={{ display: "flex", alignItems: "center", gap: "0.75rem", cursor: "pointer", flex: 1 }}>
            ...
            <div>
              <div style={{ fontFamily: "var(--font-body)", fontSize: "0.88rem", fontWeight: 600, color: "white" }}>
                Show Achievements section on homepage
              </div>
              <div style={{ fontFamily: "var(--font-body)", fontSize: "0.75rem", color: "rgba(255,255,255,0.45)", marginTop: "0.2rem" }}>
                {sectionEnabled
                  ? "Achievements section is visible on the public homepage."
                  : "Achievements section is hidden. The /achievements/submit form remains accessible."}
              </div>
            </div>
          </label>
